...
const { ENCRYPTION_KEY, IV_VALUE, S3_BUCKET_NAME, CLOUD_STORAGE_PROVIDER, GCS_BUCKET_NAME, AZURE_STORAGE_CONTAINER_NAME, MAIL_TRANSPORTER, SEND_EMAIL_PROVIDER } = require('../config/config')
const fs = require('fs')
...
const generateSlug = ({ sCategoryName, sLingoName, sArticleTitle, dPublishedAt }) => {
  if (!sCategoryName || !sLingoName || !sArticleTitle || !dPublishedAt) {
    throw new Error('Invalid article data')
  }

  const category = slugify(sCategoryName)
  const lingo = slugify(sLingoName)
  const title = slugify(sArticleTitle)
  const timestamp = new Date(dPublishedAt).getTime()

  return `${category}/${lingo}/${title}-${timestamp}`
}
...
const randomStr = (len, type) => {
  ...
  if (type === 'private') {
    ...
    // Default character set if no type specified
...
function generateResponse(res, statusCode, message, data){
  if(!data){
